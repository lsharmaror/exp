...
contract("ListingRewards - LISTEE PAYOUT", accounts => {
	...
	it("Listee trying to get his payout after 14 days without any flag", async () => {
		await listing
	...
	it("Listee trying to get his payout before 14 days without any flag", async () => {
		try {
			await listing
				.newRewardRequest(1, { from: listee1, value: 2 })
				.then(tx => {
			...
			await web3.currentProvider.sendAsync(
					{
						jsonrpc: "2.0",
						method: "evm_increaseTime",
						params: [86400 * 13], // 86400 seconds in a day
						id: new Date().getTime()
					},
					() => {}
				);
	...
	it("Listee trying to get payout after 14 days for listing with a flag", async () => {
		try {
			await listing
				.newRewardRequest(1, { from: listee1, value: 2 })
				.then(tx => {
	...
	it("Listee trying to get payout after 14 days for a cancelled listing", async () => {
		try {
			await listing
				.newRewardRequest(1, { from: listee1, value: 2 })
				.then(tx => {
	...
	it("Listee trying to get payout before 14 days for a cancelled listing", async () => {
		try {
			await listing
				.newRewardRequest(1, { from: listee1, value: 2 })
				.then(tx => {
